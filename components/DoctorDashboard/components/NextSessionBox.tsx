...
/** Looser parser: detects "follow up", "follow-up", "followup" (and initial/emergency) */
function sessionTypeFromTitle(title?: string): string | null {
  if (!title) return null;
  const t = title.toLowerCase();
  const norm = t.replace(/[^\w\s]/g, "").replace(/\s+/g, " "); // strip punctuation, normalize spaces
...
/** Fallback: prettify transport mode to a label */
...
/** Final label chooser: prefer explicit session_type (tag), then title, then transport */
...
export const NextSessionBox: React.FC<NextSessionBoxProps> = ({ nextSession }) => {
  ...
  return (
    <>
      ...
      >
        <h3
          className="text-3xl font-bold text-[#1E3CA7] mb-2 text-center"
        ...
        {nextSession ? (
          <>
            <div className="space-y-3 mb-8">
              <div className="flex items-center">
                <span className="text-xl text-[#1E3CA7]" style={{ fontWeight: 700 }}>
            ...
            <div className="space-y-2 mb-8">
              <p className="text-xl text-[#1E3CA7]">
                <span style={{ fontWeight: 700 }}>Patient:</span> {nextSession.patient_name}
              </p>
              <p className="text-xl text-[#1E3CA7]">
                <span style={{ fontWeight: 700 }}>Session Type:</span> {typeLabel}
              </p>
            </div>
          ...
          <div className="flex-1 flex items-center justify-center">
            <p className="text-[#1E3CA7] text-lg" style={{ fontWeight: 400 }}>
